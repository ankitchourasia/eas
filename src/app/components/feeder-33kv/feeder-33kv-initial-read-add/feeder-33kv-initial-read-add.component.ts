...
@Component({
  selector: 'eas-33kv-feeder-initial-read-add',
  templateUrl: './feeder-33kv-initial-read-add.component.html',
  styleUrls: ['./feeder-33kv-initial-read-add.component.css']
})
export class Feeder33KVInitialReadAddComponent implements OnInit {

  COMPONENT_NAME: string = "Feeder33KVInitialReadAddComponent";
  user : any = {};
  feederReading : any = {};
  regionList: any;
  circleList: any;
  ...
  zoneChanged(zoneId){
    ...
    this.clearDetails();
    this.getFeedersByZoneId(zoneId);
  }

  getFeedersByZoneId(zoneId){
    this.feeders = [];
  ...
  feederChanged(feeder){
    ...
    this.getFeederReadingByFeederId(feeder.id);
  }

  getFeederReadingByFeederId(feederId){
    ...
    this.feederService.getFeeder33KVReadingsByFeederId(feederId, true).subscribe(success =>{
