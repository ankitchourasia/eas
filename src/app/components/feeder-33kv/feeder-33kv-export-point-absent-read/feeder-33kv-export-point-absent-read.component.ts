...
@Component({
  selector: 'eas-feeder-33kv-export-point-absent-read',
  templateUrl: './feeder-33kv-export-point-absent-read.component.html',
  styleUrls: ['./feeder-33kv-export-point-absent-read.component.css']
})
export class Feeder33KVExportPointAbsentReadComponent implements OnInit {

  COMPONENT_NAME: "Feeder33KVExportPointAbsentReadComponent";
  ...
  getByDivisionId(divisionId){
    let methodName = "getByDivisionId";
    this.exportPoints = [];
    if(this.user && this.user.division){
      this.loading = true;
      this.exportService.getAbsent33KVExportReadByDivisionId(divisionId, this.billMonth, false).subscribe(successResponese =>{
        this.loading = false;
  ...
  getByZoneId(zoneId){
    ...
    if(this.user && this.user.division){
      this.loading = true;
      this.exportService.getAbsent33KVExportReadByZoneId(zoneId, this.billMonth, false).subscribe(successResponese =>{
        this.loading = false;
        this.exportPoints = successResponese;
